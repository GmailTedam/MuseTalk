...
def pull(repo_id: str, local_dir: str, allow_patterns):
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            print(
                f"[{repo_id}] attempt {attempt}/{MAX_RETRIES} -> {local_dir}",
                flush=True,
            )
            snapshot_download(
                repo_id=repo_id,
            ...
            print(
                f"[{repo_id}] FAIL attempt {attempt}: {type(exc).__name__}: {exc}",
                flush=True,
            )
